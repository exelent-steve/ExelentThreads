...
#### Fixed Columns (A-I) - ALWAYS in this order in export:
...
7. **Assignee** - Enum: `Me`, `Claude`, `Team`, `Unassigned` (determines export behavior)
8. **Date_Created** - ISO format: `YYYY-MM-DD` or `DD/MM/YYYY` (parser accepts both)
9. **Date_Updated** - ISO format: `YYYY-MM-DD` or `DD/MM/YYYY` (auto-updated by app)

#### Expandable Comment Columns (J onwards):
10. **Your_Comments** - User's first comment (optional)
11. **Claude_Comments** - AI's first response (optional)
12. **Your_Comments_2** - User's second comment (optional)
13. **Claude_Comments_2** - AI's second response (optional)
14. ... continues as needed (Your_Comments_3, Claude_Comments_3, etc.)

**Assignee Column Behavior:**
- `Claude` - Topic will be included in "Assigned to Claude" export preset
- `Me` - User is handling this topic (excluded from Claude exports by default)
- `Team` - Requires team input (excluded from Claude exports by default)
- `Unassigned` - Not yet assigned (included in "All Topics" export only)
...
- Template content:
```
CRITICAL RULES FOR AI: (1) NEVER overwrite existing comment columns - ALWAYS add a NEW column to the right. (2) Name new columns sequentially: Your_Comments, Claude_Comments, Your_Comments_2, Claude_Comments_2, etc. (3) Prefix each comment with timestamp: 'YYYY-MM-DD HH:MM: ' (4) Update Date_Updated column (Column I) to current date when modifying any row. (5) Update Status column (Column D) to Resolved when answering. (6) Fixed columns A-I are: ID, Topic, Description, Status, Priority, Category, Assignee, Date_Created, Date_Updated. (7) Parser identifies columns by NAME not position - column order can vary. (8) ID column uses UUIDs (format: topic_abc123). (9) Do NOT modify Assignee column - user controls assignment. (10) This CSV file is the TEMPLATE - use exact structure when creating exports.
```
...
2. Create map: `columnName → columnIndex`
...
If any required column missing → show error:
```
...
CREATE TABLE topics (
  ...
  assignee TEXT DEFAULT 'Unassigned',  -- Me|Claude|Team|Unassigned
...
- `assignee` CHECK: IN ('Me', 'Claude', 'Team', 'Unassigned')
...
4. For each data row:
   ...
   f. For each comment:
      ...
      - Determine author: "Your_Comments" → user, "Claude_Comments" → claude
      - Determine round: _2 suffix → round 2, etc.
      - INSERT into comments table (or UPDATE if exists)
   g. Extract categories → INSERT into categories table if not exists
5. Show success toast: "Imported X topics, Y comments"
```

### 4.2 Export Flow (Database → CSV)

**Export Selection Dialog:**

Before exporting, show modal with topic selection presets:

```html
<div class="export-selection-modal">
  <h2>Select Topics to Export</h2>
  ...
  <div class="export-actions">
    ...
    <button class="btn-export-selected">Export Selected</button>
  </div>
</div>
```

**Custom Selection:**
If user selects "Custom", show topic list with checkboxes for manual selection.

**Preset Filtering:**
- `assigned-claude`: WHERE assignee = 'Claude'
- `all-open`: WHERE status IN ('Open', 'In Progress')
- `all-topics`: SELECT * (no filter)
- `custom`: User-selected topic IDs
...
5. For each topic row:
   ...
   - For each comment:
     - Determine column: author + round → Your_Comments_2, etc.
...
5. User clicks "Import" → trigger import flow
...
```
┌─────────────────────────────────────────────────────────────┐
│  Header: Logo | Search Bar | Actions (Import, Export, +New) │
├──────────┬──────────────────────────────────────────────────┤
│          │                                                   │
...
- Click project → switch to that project (reload topics from DB)
- Manage Projects → modal to add/remove/edit projects
...
<header class="content-header">
  ...
  <div class="search-container">
    <input type="text" id="global-search" placeholder="🔍 Search topics..." />
  </div>
...
│  Topic Card 1                                       │
│  ┌────────────────────────────────────────────────┐ │
│  │ 🔗 [Related Topics: Topic 5, Topic 12]         │ │
│  │                                                 │ │
│  │ **API Authentication Strategy**    [Resolved]  │ │
│  │ Need to decide between JWT and session-based   │ │
│  │                                                 │ │
│  │ 💬 3 exchanges | 📁 Security | ⬆️ High          │ │
│  │                                                 │ │
│  │ Latest: "2025-11-04: Decided on JWT..."        │ │
│  │ [View Details]                                  │ │
│  └────────────────────────────────────────────────┘ │
│                                                      │
│  Topic Card 2                                       │
...
│  │ PostgreSQL vs MySQL for main database          │ │
│  │                                                 │ │
│  │ 💬 2 exchanges | 📁 Database | → Medium         │ │
│  │                                                 │ │
│  │ Latest: "2025-11-03: Considering Postgres..."  │ │
│  │ [View Details]                                  │ │
│  └────────────────────────────────────────────────┘ │
│                                                      │
└─────────────────────────────────────────────────────┘
...
- Click card OR "View Details" → open Topic Detail Modal
...
<div class="topic-card" data-topic-id="{topic.id}">
  ...
  <div class="topic-card-meta">
    <span>💬 {topic.commentCount} exchanges</span>
    <span>📁 {topic.category}</span>
    <span class="priority-{topic.priority.toLowerCase()}">
      {#if topic.priority === 'High'}⬆️{:else if topic.priority === 'Medium'}→{:else}↓{/if}
      {topic.priority}
    </span>
  </div>
...
**Last Responder Indicator:**

Show who replied last to help track "whose turn" it is:

```html
<div class="topic-card-last-responder">
  {#if topic.lastResponder === 'user'}
    <span class="responder-badge responder-you">⬅️ You replied last</span>
  {:else if topic.lastResponder === 'claude'}
    <span class="responder-badge responder-claude">➡️ Claude replied last</span>
  {:else}
    <span class="responder-badge responder-none">No responses yet</span>
  {/if}
</div>
```
...
**Logic:**
```typescript
const determineLastResponder = (comments: Comment[]): 'user' | 'claude' | 'none' => {
  if (comments.length === 0) return 'none';

  const lastComment = comments[comments.length - 1];
  return lastComment.author; // 'user' or 'claude'
};
```
...
```
┌────┬────────────────────┬───────────┬─────────┬──────────┬─────────┐
│ ID │ Topic              │ Status    │ Priority│ Category │ Updated │
├────┼────────────────────┼───────────┼─────────┼──────────┼─────────┤
│ 1  │ API Auth Strategy  │ Resolved  │ High    │ Security │ Nov 4   │
│ 2  │ Database Choice    │ Open      │ Medium  │ Database │ Nov 3   │
│ 3  │ Error Handling     │ In Prog   │ High    │ Backend  │ Nov 5   │
└────┴────────────────────┴───────────┴─────────┴──────────┴─────────┘
```

**Features:**
- Click column header → sort ascending/descending
- Click row → open Topic Detail Modal
...
<table class="topics-table">
  <thead>
    <tr>
      ...
      <th data-sort="date_updated">Updated <span class="sort-icon">↕</span></th>
    </tr>
  </thead>
  <tbody>
...
├──────────────┼──────────────┼──────────────┼──────────────┤
│ ┌──────────┐ │ ┌──────────┐ │ ┌──────────┐ │ ┌──────────┐ │
│ │ Topic 1  │ │ │ Topic 3  │ │ │ Topic 2  │ │ │ Topic 5  │ │
│ │ High     │ │ │ High     │ │ │ Med      │ │ │ Low      │ │
│ │ Security │ │ │ Backend  │ │ │ Database │ │ │ UX       │ │
│ └──────────┘ │ └──────────┘ │ └──────────┘ │ └──────────┘ │
│              │              │              │              │
│ ┌──────────┐ │              │ ┌──────────┐ │              │
│ │ Topic 4  │ │              │ │ Topic 6  │ │              │
...
- Click card → open Topic Detail Modal
...
**Layout:**
```
Nov 5, 2025  ─┬─ [Topic 3: Error Handling] In Progress | High | Backend
              │
Nov 4, 2025  ─┼─ [Topic 1: API Auth] Resolved | High | Security
              │
Nov 3, 2025  ─┼─ [Topic 2: Database] Open | Medium | Database
              │   [Topic 4: UI Components] Open | Low | Frontend
              │
Nov 2, 2025  ─┴─ [Topic 6: Deployment] Closed | Med | DevOps
```
...
- Click topic → open Topic Detail Modal
...
│                                                          │
│ ┌───────────────────────────────────────────┐           │
│ │ Topics by Status (Pie Chart)              │           │
│ │                                            │           │
│ │     [Pie Chart Visualization]             │           │
│ └───────────────────────────────────────────┘           │
│                                                          │
│ ┌───────────────────────────────────────────┐           │
│ │ Topics by Category (Bar Chart)            │           │
│ │                                            │           │
│ │ Technical   ████████████ 15               │           │
│ │ UX          ███████ 10                    │           │
│ │ Database    ████ 6                        │           │
...
│ │                                            │           │
│ │     [Line Chart: Topics created/updated]  │           │
│ └───────────────────────────────────────────┘           │
└─────────────────────────────────────────────────────────┘
```
...
│   ├── projects/
│   │   ├── [id]/
│   │   │   ├── page.tsx           # Project view (renders active view)
│   │   │   ├── conversation/      # Conversation view component
│   │   │   ├── table/             # Table view component
│   │   │   ├── board/             # Board view component
│   │   │   ├── timeline/          # Timeline view component
│   │   │   └── analytics/         # Analytics view component
│   │   └── new/
│   │       └── page.tsx           # New project creation
│   ├── api/
│   │   ├── import/
│   │   │   └── route.ts           # Import CSV endpoint
│   │   ├── export/
│   │   │   └── route.ts           # Export CSV endpoint
│   │   └── topics/
│   │       ├── route.ts           # CRUD topics
│   │       └── [id]/
│   │           └── route.ts       # Single topic operations
...
│   ├── modals/
│   │   ├── new-topic-modal.tsx    # New topic form
...
│  Description:                                       │
│  Need to decide between JWT and session-based       │
│  authentication for the API endpoints. Consider     │
│  security, scalability, and team familiarity.       │
│                                                      │
│  📁 Security | ⬆️ High | 📅 Created: Nov 3, 2025    │
│  ────────────────────────────────────────────────── │
│                                                      │
│  💬 Conversation Thread:                            │
│                                                      │
│  ┌────────────────────────────────────────────────┐ │
│  │ 👤 Your Comment (Nov 3, 2025 10:30)            │ │
│  │ What are the pros and cons of JWT vs session?  │ │
│  └────────────────────────────────────────────────┘ │
│                                                      │
│  ┌────────────────────────────────────────────────┐ │
│  │ 🤖 Claude's Response (Nov 3, 2025 10:35)       │ │
│  │ JWT pros: Stateless, scalable...               │ │
│  │ Session pros: Revocable, simpler...            │ │
│  └────────────────────────────────────────────────┘ │
│                                                      │
│  ┌────────────────────────────────────────────────┐ │
│  │ 👤 Your Comment (Nov 4, 2025 14:20)            │ │
│  │ Let's go with JWT for our use case.            │ │
│  └────────────────────────────────────────────────┘ │
│                                                      │
│  [Add Your Comment]                                 │
│  ┌────────────────────────────────────────────────┐ │
...
**Locked Fields:**
- Topic and Description show 🔒 icon if `is_locked = 1`
- Hover tooltip: "Locked after first export to prevent sync issues"

**Comment Thread:**
- Display all comments chronologically
- User comments: right-aligned, light background, 👤 icon
- Claude comments: left-aligned, dark background, 🤖 icon
...
- Close (×): discard unsaved changes (confirm if dirty)
    ...
    <div className="topic-modal">
      ...
      <div className="topic-modal-content">
        ...
        <div className="topic-modal-header">
          ...
          />
          {topic.is_locked && <span title="Locked after first export">🔒</span>}
          ...
          <button onClick={onClose} className="topic-modal-close">✕</button>
        </div>
        ...
        <div className="topic-modal-body">
          ...
          <div className="topic-modal-meta">
            <span>📁 {topic.category}</span>
            <span>📅 Created: {formatDate(topic.date_created)}</span>
            <span>📅 Updated: {formatDate(topic.date_updated)}</span>
          </div>
